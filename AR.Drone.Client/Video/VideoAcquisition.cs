            ...
            using (NetworkStream stream = tcpClient.GetStream())
            {
                ...
                fixed (byte* pBuffer = &buffer[0])
                    while (token.IsCancellationRequested == false)
                    {
                        ...
                        if (packetData == null)
                        {
                            ...
                            for (int i = 0; i < maxSearchIndex; i++)
                            {
                                if (buffer[i] == 'P' && buffer[i + 1] == 'a' && buffer[i + 2] == 'V' && buffer[i + 3] == 'E')
                                {
                                    ...
                                    packetData = new sbyte[pve.payload_size];
                        ...
                        {
                        // frame acquired
                        var signed = new sbyte[buffer.Length];
                        Buffer.BlockCopy(buffer, 0, signed, 0, buffer.Length);
                        Array.Copy(signed, frameStart, packetData, 0, packetData.Length);
                            //Array.Copy((sbyte[])((Array)buffer), frameStart, packetData, 0, packetData.Length);
